...
> 案例：证明向量集是 subspace
> 
> 例：线性变换 $T:\mathbb{R}^n \rightarrow \mathbb{R}^m$。
> 求证：若 $V$ 是 $\mathbb{R}^n$ 的 subspace，则 $W=\{T(\mathbf{u})\mid \mathbf{u}\in V\}$ 是 subspace。
> 
> 证明：$T$ 是线性系统，满足 $T(\mathbf{u}_1+\mathbf{u}_2)=T(\mathbf{u}_1)+T(\mathbf{u}_2)$，$cT(\mathbf{u})=T(c\mathbf{u})$。
> ①$W$ 显然非空
> ②任取 $\mathbf{w}_1,\mathbf{w}_2\in W$，则 $\mathbf{w}_1=T(\mathbf{u}_1)$，$\mathbf{w}_2=T(\mathbf{u}_2)$
> 则 $\mathbf{w}_1+\mathbf{w}_2=T(\mathbf{u}_1)+T(\mathbf{u}_2)=T(\mathbf{u}_1+\mathbf{u}_2)\in W$（因为 $\mathbf{u}_1+\mathbf{u}_2\in V$）
> ③任取 $\mathbf{w}\in W$，则 $\mathbf{w}=T(\mathbf{u})$
> 则 $c\mathbf{w}=cT(\mathbf{u})=T(c\mathbf{u})\in W$（因为 $c\mathbf{u}\in V$）
> 综上，$W$ 是 subspace。
...
> 案例：证明向量集是 basis
> 
> 例：设 $A=\{\mathbf{u}_1,\mathbf{u}_2,\cdots,\mathbf{u}_k\}$ 是 subspace $V\subseteq \mathbb{R}^n$ 的 basis。
> 求证：$\{\mathbf{v},\mathbf{u}_2,\cdots,\mathbf{u}_k\}$ 也是 $V$ 的 basis，其中 $\mathbf{v}=\mathbf{u}_1+\mathbf{u}_2+\cdots+\mathbf{u}_k$。
> 
> 证明：$\{\mathbf{v},\mathbf{u}_2,\cdots,\mathbf{u}_k\}$ 有 $k$ 个向量，因此只要证它是 $V$ 的 independent 子集，即可证明它是 $V$ 的 basis。
> ①证明是子集：$\{\mathbf{v},\mathbf{u}_2,\cdots,\mathbf{u}_k\}$ 中的每个元素都 $\in V$，因此是子集
> ②证明 independent：只要证 $c_1\mathbf{v}+c_2\mathbf{u}_2+\cdots+c_k\mathbf{u}_k=\mathbf{0}$ 的唯一解是 $c_1=c_2=\cdots=c_k=0$。原方程 $\iff$ $c_1(\mathbf{u}_1+\mathbf{u}_2+\cdots+\mathbf{u}_k)+c_2\mathbf{u}_2+\cdots+c_k\mathbf{u}_k=\mathbf{0}$ $\iff$ $c_1\mathbf{u}_1+(c_1+c_2)\mathbf{u}_2+\cdots+(c_1+c_k)\mathbf{u}_k=\mathbf{0}$。由 $A$ 是 basis 知 $\{\mathbf{u}_1,\mathbf{u}_2,\cdots,\mathbf{u}_k\}$ independent，因此原方程解得 $c_1=c_1+c_2=\cdots=c_1+c_k=0$，即 $c_1=c_2=\cdots=c_k=0$。
