...
该方法适用于同一个矩阵解很多次的情况（$A\mathbf{x}=\mathbf{b}_1,A\mathbf{x}=\mathbf{b}_2,\dots$），先求 $A^{-1}$，再对不同的 $\mathbf{b}$ 套用 $\mathbf{x}=A^{-1}\mathbf{b}$。应用案例见[[2  coordinate system#Change Coordinate]]。
